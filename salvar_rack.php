...
try {
  ...
  if ($id <= 0 || !in_array($field, $allowed, true)) {
    ...
    echo json_encode(['success'=>false, 'msg'=>'Parâmetros inválidos', 'debug'=>$DEBUG ? compact('id','field') : null]); exit;
  }
  ...
  if (!$okPing) {
    ...
    echo json_encode(['success'=>false, 'msg'=>'DB indisponível', 'debug'=>$DEBUG ? ['db_error'=>$err] : null]); exit;
  }
